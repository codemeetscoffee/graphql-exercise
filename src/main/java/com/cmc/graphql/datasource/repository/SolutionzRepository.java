...
public interface SolutionzRepository extends JpaRepository<Solutionz, UUID> {
    @Query(nativeQuery = true, value = "select * from solutionz " +
    ...
    @Query(nativeQuery = true,
            value = "update solutionz set vote_bad_count + 1 where id = :solutionzId"
    )
    ...
    @Modifying
    @Query(nativeQuery = true,
            value = "update solutionz set vote_good_count + 1 where id = :solutionzId"
    )
